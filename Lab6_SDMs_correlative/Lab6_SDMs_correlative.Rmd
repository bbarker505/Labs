---
author: "Your name"
title: "Lab 6: Species distribution modeling  (correlative)"
...
knitr::include_graphics(c(here("Lab6_SDMs_correlative", "images", "hooded-warbler-female.jpg"),
                          here("Lab6_SDMs_correlative", "images", "hooded-warbler-male.jpg")))
```


```{r, echo=FALSE, outwidth = "400px", fig.cap="Range map for the hooded warbler"}
knitr::include_graphics(here("Lab6_SDMs_correlative", "images", "hooded-warbler-range-map.jpg"))
```
...
As discussed during class, there are many different algorithms available to model a species' distribution. For this exercise, we will use generalized linear modeling (GLM), which is one of the simplest. The Biodiversity and Climate Change Virtual Laboratory (BCCVL) has a good summary of correlative SDM algorithms including the [GLM method](https://support.bccvl.org.au/support/solutions/articles/6000083213-generalized-linear-model). We'll build a multivariate logistic regression model using the [`glm()`](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/glm) function in base R.  
...
**(1)** In the code chunk below, create an object named `log_regr_model` with the three necessary arguments. 
...
A summary of the GLM can obtained using the `summary()` function. 
...
**(3)** Interpret the GLM model predictions in relation to presence-absence locations. Where is the probability of presence highest vs. lowest? Why do you think the hooded warbler might be absent from certain areas where the probability of presence is high?
...
knitr::include_graphics(here("Lab6_SDMs_correlative", "images", "threshold_0.5.png"))
```

**(4)** In the code chunk below, add code to subset predictions that have a probability of greater than `0.5`. Next, copy and paste the code used to map probability of presence, but instead plot the subsetted predictions. Change the title of your plot to "GLM - Presence-Absence Map for Hooded Warbler (Threshold = 0.5)". 
...
```{r, echo=FALSE, fig.cap ="Source: BCCVL (https://support.ecocommons.org.au/support/solutions/articles/6000163162-model-evaluation)"}
knitr::include_graphics(here("Lab6_SDMs_correlative", "images", "ROC_curve.png"))
```
...
**(5)** Interpret the ROC curve. How do you know that your model performs better than random? What does the AUC value tell you? 
...
**(6)** In the code chunk below, copy and paste the code for **Q4**, but replace the threshold of `0.5` with the threshold estimated above (use a new column name, such as `thresh_prev`). Additionally, create a new plot named `binary_map2` to show the new presence-absence map. After you're done, answer the question below.
...
# Save the plot
ggsave(SDM_forecast, filename = here("Lab6_SDMs_correlative", "maps", "warbler_SDM_forecast.png"), dpi = 150, units = c('in'), width = 10, height = 6)
...
**(7)** In the code chunk below, create a map to visualize changes in the probability of presence of hooded warbler under the CMIP5 50-year climate projections. You can name it `SDM_differences` and use "GLM - Change in Probability of Hooded Warbler Presence (Future - Current)" as the title. After you're done, answer the question below.

```{r}
# Plot differences 
# Can name it "warbler_SDM_differences"
...
# Save the plot
ggsave(error_plot, filename = here("Lab6_SDMs_correlative", "maps", "warbler_extrapolation_errors.png"), dpi = 150, units = c('in'), width = 10, height = 6)

# View plot
error_plot
```

**(8)** Where did model extrapolation errors occur according to the MESS analysis? What might be the consequence of interpreting model predictions for these areas?

*Response*: 



**(9)** Describe at least 3 major differences between correlative SDM and process-based SDM approaches, making sure to point out if these differences have advantages and/or disadvantages.

*Response*:



**(10)** Discuss your overall results from the correlative SDM modeling lab.  
