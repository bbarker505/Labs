---
author: "Your name"
title: "Lab 7: Predator-prey models"
...
- Provide examples of some stabilizing factors in predator-prey or host-parasitoid interactions and how they allow prey/host population to persist in the long-term  
...
knitr::include_graphics(here("Lab7_predator_prey_model", "images", "braconid_wasp.png"))
```
...
knitr::include_graphics(here("Lab7_predator_prey_model", "images", "parasitoid_wasp_life_cycle.png"))
```
...
After defining equations to calculate $H$ and $P$, a data frame with three columns (`nb_out_all`) is created to hold model outputs. The first row is for year 0 (`Time = 0`), which is calculated using the initial values. A `for` loop iterates over each year in `time` and calculates $H$ and $P$. These results are added to the `nb_out_all` data frame as a new row. Finally, an if-else statements is used to round up to the nearest integer using `ceiling()` to avoid having fractions of individuals. 

```{r}
# Simple Nicholson-Bailey model
# Agurments are # of time steps, pop growth rate, search efficiency, and search area.
...
The `nb_model_df1` data frame contains host and parasitoid outputs for each year.
```{r}
...
A plot depicting the dynamics of host and parasitoid densities over time demonstrates how the simple Nicholson-Bailey model leads to mutual extinction of host and parasitoid. Realistically, there shouldn't even be a second peak in the plot because densities drop to 0 at year 3 (host) or year 4 (parasitoid). 
...
Plotting host vs. parasitoid densities reveals how the trajectory spirals outwards until the population crashes.
...
**(1)** In the code chunk below, run the model using starting conditions that reflect a situation in which a parasitoid oviposits one egg per host, has a searching efficiency of 75%, and a reproductive rate of `2`. Run the model for `30` generations. Create plots of the model output (i.e., using the `Plot_DensVTime` and `Plot_DensVDens` functions). Make sure to create a new plot title.
...
plot_densVdens2 
ggsave(plot_densVtime2, filename = here("Lab7_predator_prey_model", "plots", "NB_model_densVtime_Q1.png"), height = 4, width = 5, units = c('in'), device = "png", dpi = 150)
ggsave(plot_densVdens2, filename = here("Lab7_predator_prey_model", "plots", "NB_model_densVdens_Q1.png"), height = 4, width = 5, units = c('in'), device = "png", dpi = 150)
```

**(2)** Use the same parameters as before except change the searching efficiency to 5%, and then answer the question below.
...
plot_densVdens3 
ggsave(plot_densVtime3, filename = here("Lab7_predator_prey_model", "plots", "NB_model_densVtime_Q2.png"), height = 4, width = 5, units = c('in'), device = "png", dpi = 150)
ggsave(plot_densVdens3, filename = here("Lab7_predator_prey_model", "plots", "NB_model_densVdens_Q2.png"), height = 4, width = 5, units = c('in'), device = "png", dpi = 150)
```
...
**(3)** Repeat this process for the other two parameters, reproductive rate (`R`) and number of hatched parasitoids (`c`), but choose your own values this time. See the comments below for naming conventions (avoids confusion later). Then answer the question below.
...
**(4)** Why does the simple Nicholson-Bailey model show unstable oscillations and eventually lead to mutual population extinction?
...
**(5)** In the empty code chunk below, run your model and use the two plotting functions to create plots of model outputs. Use the below initial values for this question and the next. The plot title should now have the `K` parameter (see below comment).
...
ParamRunner <- function(param, values) {
  ...
  ggsave(allPlots, filename = here("Lab7_predator_prey_model", "plots", outfile),
        height = 15, width = 15, units = c('in'), device = "png", dpi = 150)
}
```

**(6)** Explore how densities of the host and parasitoid change over time when changing:   
...
**(6c)** Varying $a$. FINAL EXAM.

```{r}
# Changing search efficinecy parameter "a"
time <- 100
...
R <- 2
c <- 0.5
a_values <-  seq(from =  0.0001, to = 0.001, by = 0.00015)
a_values <-   seq(from =  0.00005, to = 0.0003, by = 0.00005)
K <- 50000
K <- 40000
K <- 30000
K <- 20000
...
**(7)** Summarize how variation in each of the 4 parameters ($R$, $c$, $a$, and $K$) affects how host and parasitoid densities change over time. Keep in mind that y-axis limits may differ across the plots produced for each parameter value.  

*Response*: 


**(8)** Does your modified Nicholson-Bailey model still show unstable oscillations and eventually lead to mutual population extinction? Did some parameter combinations result in greater stability? Explain. 

*Response*: 


**(9)** Which factor would you add next to stabilize the model and why?

*Response*: 


**(10)** Discuss your overall results from the Nicholson-Bailey model simulations. In your response, describe some potential applications of the model, such as for biological control programs of insect pests. What qualities would you look for in the parasitoid?
