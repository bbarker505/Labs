...
# Run the model
nb_model_df1 <- NB_model(time = time, R = R, c = c, a = a)
```
...
**(1)** In the code chunk below, run the model using starting conditions that reflect a situation in which a parasitoid oviposits one egg per host, has a searching efficiency of 75%, and a reproductive rate of `2`. Run the model for `30` generations (years). Create plots of the model output (i.e., using the `Plot_DensVTime` and `Plot_DensVDens` functions). Make sure to create a new plot title.

```{r}
# Initial conditions
# c <- ? No. of parasitoids that hatch from infected host
# a <- ? # Searching efficiency of female parasitoid 
# R <- ? # Reproductive rate (number of surviving host offspring)
#time <- ? # No. of generations (years)
...
**(5)** In the empty code chunk below, run your model and use the two plotting functions to create plots of model outputs. Use the below initial values for this question and the next. 
...
# Keep other param values the same as in 6a
# c_values <- ?
...
```{r}
# Changing K (host density dependence)
# Keep other param values the same as in 6a
# K_values <- ?
