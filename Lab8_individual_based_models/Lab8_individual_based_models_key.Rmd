...
- Produce and interpret outputs of an individual-based movement model   
- Describe the influence of random and non-random components in a model  
- Modify code to change the behavior of an individual-based model     
...
```{r}
sim_2 <- CM_time_step(n_male = 5, n_female = 5, n_step = 100, wind_card = "N")
sim2_plot <- Movement_plot(sim_2, "sim2")
sim2_plot
```

Wind direction = south
```{r}
sim_3 <- CM_time_step(n_male = 5, n_female = 5, n_step = 100, wind_card = "S")
sim3_plot <- Movement_plot(sim_3, "sim3")
sim3_plot
```

Wind direction = west
```{r}
sim_4 <- CM_time_step(n_male = 5, n_female = 5, n_step = 100, wind_card = "W")
sim4_plot <- Movement_plot(sim_4, "sim4")
sim4_plot
```

Wind direction = east
```{r}
sim_5 <- CM_time_step(n_male = 5, n_female = 5, n_step = 100, wind_card = "E")
sim5_plot <- Movement_plot(sim_5, "sim5")
sim5_plot
```

Wind direction = southeast
```{r}
sim_6 <- CM_time_step(n_male = 5, n_female = 5, n_step = 100, wind_card = "SE")
sim6_plot <- Movement_plot(sim_5, "sim6")
sim6_plot
```

**(1)** Explain at least two reasons why your simulation with north wind did not produce the exact same results as before.

*Response*: Stochastic/random elements of the model explain why outputs differ across runs. Starting points are random. Flight velocity for bot sexes is randomly sampled from a uniform distribution. Direction of flight for males is randomly sampled from a Gaussian distribution centered on the direction of the previous step, whereas direction of flight for females is randomly sampled from a uniform distribution.

**(2)** What does casting behavior in males look like in your output plots? 

*Response:* Males always fly in a cross-wind direction regardless of wind direction.
...
*Response:* Results will vary, but females should always stay in a small area compared to males, which have more bias in their movement (correlated random walk) due to their flying in a cross-wind direction and going in a direction that is limited to within about 40 degrees from their direction in the previous time step. Females move according to a complete random walk.  
