...
*Response*: Using a threshold of 50F resulted in more DDs than a threshold of 54F because a smaller number is being subtracted from the average of tmax and tmin. Biologically, this means that an organism can begin developing at colder temperatures. 
...
*Response*: The min is 0 and the max is 2595.8 DDs.
...
weath_tbl <- read_table(file = here("Lab2_phenology_models", "weather_data", "CRVO21.txt"),
...
*Response*: We looped through years (n = 10) as well as the number of days (N = 365) for each year. For each year, degree-day accumulation is calculated. Approximately 10X more code would be required to run the daily time step separately for each year, which violates the DRY principle. 
